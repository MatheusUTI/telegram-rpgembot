...
def _gerar_item_magico(config, id_item_base, raridade_maxima):
    ...
    item_gerado.update({'nome_exibido': nome_final, 'descricao_final': f"{base_item['descricao_base']}\n*Efeitos Mágicos:* {'; '.join(lista_efeitos_desc)}", 'efeitos': lista_efeitos_desc, 'raridade': raridade_final})
    return item_gerado
...
def _processar_tabela_de_loot(config, player_ref, tabela_id):
    ...
    for drop in tabela.get('drops', []):
        if random.randint(1, 100) <= drop.get('chance', 0):
            ...
            if item_gerado:
                loot_encontrado_texto.append(f"✨ {telegram_actions.escape_markdown_v2(item_gerado['nome_exibido'])} `({item_gerado['raridade']})`")
...
def handle_start_command(config, user_id, chat_id, player_doc):
    if player_doc.exists and 'ficha' in player_doc.to_dict():
        ...
        nome_escapado = telegram_actions.escape_markdown_v2(nome_personagem)
        texto = f"⚔️ Bem\\-vindo\\(a\\) de volta, *{nome_escapado}*\\! Sua aventura nas Terras de Aethel continua\\."
        telegram_actions.send_telegram_message(config.TELEGRAM_TOKEN, chat_id, texto)
    else:
        telegram_actions.send_telegram_message(config.TELEGRAM_TOKEN, chat_id, "Sua lenda ainda não foi escrita\\. Use /criar\\_personagem para forjar seu destino\\.")
...
def handle_ficha_command(config, user_id, chat_id, player_doc):
    ...
    if player_doc.exists and 'ficha' in player_doc.to_dict():
        ...
        telegram_actions.send_telegram_message(config.TELEGRAM_TOKEN, chat_id, "Você precisa criar um personagem primeiro\\! Use o comando /criar\\_personagem\\.")
...
def handle_adventure_message(config, user_id, chat_id, user_text, player_ref, player_data):
    ...
    if resposta_arbitro == "SIM":
        ...
        texto_seguro = telegram_actions.escape_markdown_v2(resposta_narrador)
...
def handle_adventure_callback(config, user_id, chat_id, message_id, callback_data, player_ref, player_data):
    if callback_data == 'roll_d20':
        ...
        if acao_pendente:
            ...
            texto_seguro = telegram_actions.escape_markdown_v2(texto_narracao_limpo)
            telegram_actions.send_telegram_message(config.TELEGRAM_TOKEN, chat_id, texto_seguro)
            ...
            if loot_tag_match:
                ...
                if mensagem_loot:
                    telegram_actions.send_telegram_message(config.TELEGRAM_TOKEN, chat_id, telegram_actions.escape_markdown_v2(mensagem_loot))
